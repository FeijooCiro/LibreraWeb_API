"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = __importDefault(require("express"));
const cors_1 = __importDefault(require("cors"));
const path_1 = __importDefault(require("path"));
const connection_1 = __importDefault(require("../connection/connection"));
const index_routes_1 = __importDefault(require("../routes/index.routes"));
const listarcompleto_routes_1 = __importDefault(require("../routes/listarcompleto.routes"));
const libros_routes_1 = __importDefault(require("../routes/libros.routes"));
class Server {
    constructor() {
        this.paths = {
            index: '/index',
            listarcompleto: '/libros/listado',
            libros: '/libros'
        };
        this.app = (0, express_1.default)();
        this.port = process.env.PORT || '3000';
        // Métodos iniciales
        this.views();
        this.dbConnection();
        this.middlewares();
        this.routes();
    }
    dbConnection() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                yield connection_1.default.authenticate();
                console.log('Database online');
            }
            catch (error) {
                throw new Error(String(error));
            }
        });
    }
    routes() {
        this.app.use(this.paths.index, index_routes_1.default);
        this.app.use(this.paths.listarcompleto, listarcompleto_routes_1.default);
        this.app.use(this.paths.libros, libros_routes_1.default);
    }
    middlewares() {
        this.app.use((0, cors_1.default)());
        this.app.use(express_1.default.json());
        this.app.use(express_1.default.static('public'));
    }
    listen() {
        this.app.listen(this.port, () => {
            console.log(`Servidor corriendo en el puerto ${this.port}`);
        });
    }
    views() {
        this.app.set('views', path_1.default.join(__dirname, 'public', 'views'));
        this.app.set('view engine', 'hbs');
    }
}
exports.default = Server;
//# sourceMappingURL=app.js.map